...
def predictTeamDay():
    ...
    for i in homePIDList:
        ...
        try:
            hPStats = getPitcher("lastmonth", i[1])
    ...
    for i in range(0, len(sortedHP)):
        ...
        print("IP: " + str(sortedHP[i][0][1][2]) + " BF: " + str(sortedHP[i][0][1][3]) + " BB%: " + str(sortedHP[i][0][1][4]) + " K%: " + str(sortedHP[i][0][1][5]) + " FIP: " + str(sortedHP[i][0][1][6]) + " ERA: " + str(sortedHP[i][0][1][7]) + " WHIP: " + str(sortedHP[i][0][1][8]) + " K: " + str(sortedHP[i][0][1][10]) + " HR: " + str(sortedHP[i][0][1][11]))
    ...
    for i in awayPIDList:
        # Get the pitchers stats over the last month if available, if not then just add player's name and id
        try:
            aPStats = getPitcher("lastmonth", i[1])
    ...
    for i in range(0, len(sortedAP)):
        ...
        print("IP: " + str(sortedAP[i][0][1][2]) + " BF: " + str(sortedAP[i][0][1][3]) + " BB%: " + str(sortedAP[i][0][1][4]) + " K%: " + str(sortedAP[i][0][1][5]) + " FIP: " + str(sortedAP[i][0][1][6]) + " ERA: " + str(sortedHP[i][0][1][7]) + " WHIP: " + str(sortedAP[i][0][1][8]) + " K: " + str(sortedAP[i][0][1][10]) + " HR: " + str(sortedAP[i][0][1][11]))
    ...
    teamBatters = getBatters("lastweek", 3)
    # Print every batter in teamBatters
    printBatters('lastweek', teamBatters)


def printBatters(type, passedDict):
    """
...
def printPitchers(type, passedDict):
    """
...
def getBatters(type, status):
    ...
    for i in range(0, len(allBID)):
        ...
        for j in range(0, len(batterStats)):
            ...
            try:
                wOBA = ((.691 * float(playerBB)) + (.722 * float(playerHBP)) + (.883 * float(player1B)) + (1.256 * float(player2B)) + (
                        1.592 * float(player3B)) + (2.05 * float(playerHR))) / (playerAB + playerBB - playerIBB + playerSF + playerHBP)
                wOBA = "{:.3f}".format(wOBA)
            except: wOBA = 0
            ...
            try:
                ...
                if playerTeam in nlTeamList:
                    wRCP = ((((float(wRAA) / float(playerPA)) + .118) + (.118 - (float(playerParkFactor) * .118))) / (nlWRC / nlPA)) * 100
                else:
                    wRCP = ((((float(wRAA) / float(playerPA)) + .118) + (.118 - (float(playerParkFactor) * .118))) / (alWRC / alPA)) * 100
            except:
                wRAA = 0
                wRCP = 0
            ...
            if status == 1:
                takenBatterDict.update(
                    {playerName: [playerPA, float(bb), float(k), float(BABIP), float(playerAVG), float(playerOPS), float(wOBA), float(wRC), float(playerWAR), float(wRCP), float(wRCPA)]})
            elif status == 2:
                faBatterDict.update(
                    {playerName: [playerPA, float(bb), float(k), float(BABIP), float(playerAVG), float(playerOPS), float(wOBA), float(wRC), float(playerWAR), float(wRCP), float(wRCPA)]})
            elif status == 3:
...
def getBatter(type, playerID):
    ...
    return {playerName: [playerPA, float(bb), float(k), float(BABIP), float(playerAVG), float(playerOPS), float(wOBA),
                          float(wRC), float(playerWAR), float(wRCP), float(wRCPA)]}

def getPitcher(type, playerID):
    """
    ...
    return {playerName: [float(playerID), float(playerGames), float(playerIP), float(playerBF), float(bb), float(kP), float(FIP), float(playerERA), float(playerWHIP), float(playerWAR), float(strikeouts), float(playerHR)]}


def getPitchers(type, status):
    """
        ...
        for j in range(0, len(pitcherStats)):
            ...
            if status == 1:
                takenPlayerPDict.update(
                    {playerName: [playerID, playerGames, playerIP, playerBF, bb, kP, FIP, playerERA, playerWHIP, playerWAR, strikeouts, playerHR]})
            elif status == 2:
...
def getAllPlayers(type):
    teamPitchers = getPitchers(type, 3)
    print("All pitchers currently on your team:")
    printPitchers(type, teamPitchers)

    takenPitchers = getPitchers(type, 1)
    print("All pitchers currently on teams:")
    printPitchers(type, takenPitchers)

    faPitchers = getPitchers(type, 2)
    print("All free-agent pitchers")
    printPitchers(type, faPitchers)

    teamBatters = getBatters(type, 3)
    print("All batters currently on your team:")
    printBatters(type, teamBatters)

    takenBatters = getBatters(type, 1)
    print("All batters currently on teams:")
    printBatters(type, takenBatters)

    faBatters = getBatters(type, 2)
    print("All free-agent batters")
    printBatters(type, faBatters)
